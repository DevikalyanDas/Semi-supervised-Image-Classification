import sys
sys.path.insert(0, '/netscratch/devidas/nnti/task_1')
...
#     create_dir(saved_weights_dir)
    ...
    with torch.no_grad():
        for batch_idx, (x_l, y_l) in enumerate(test_loader):
            ...
            if batch_idx == 10:
                save_test_samples(x_l.detach().cpu(), y_l.detach().cpu(), torch.argmax(pred_prob,dim=1).detach().cpu(), img_dir, task)
